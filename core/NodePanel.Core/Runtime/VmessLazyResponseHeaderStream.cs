...
internal sealed class VmessLazyResponseHeaderStream : Stream, IInnerStreamAccessor
{
    private readonly Stream _innerStream;
    private readonly VmessRequest _request;
    ...
    public VmessLazyResponseHeaderStream(
        Stream innerStream,
    ...
    private void EnsureResponseHeader()
    {
        ...
        try
        {
            ...
            try
            {
                ...
                VmessHandshakeWriter.ReadResponseAsync(_innerStream, _request, token)
                    .AsTask()
                    .GetAwaiter()
                    .GetResult();
            }
    ...
    private async ValueTask EnsureResponseHeaderAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                await VmessHandshakeWriter.ReadResponseAsync(_innerStream, _request, token).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
                _handshakeFailure = ExceptionDispatchInfo.Capture(ex);
