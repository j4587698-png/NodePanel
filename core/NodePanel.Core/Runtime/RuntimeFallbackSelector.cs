...
internal static class RuntimeFallbackSelector
{
    public static IRuntimeFallbackDefinition? Select(
        IReadOnlyList<IRuntimeFallbackDefinition> fallbacks,
        string serverName,
        string alpn,
    ...
    private static Dictionary<string, Dictionary<string, Dictionary<string, IRuntimeFallbackDefinition>>> BuildLookup(
        IReadOnlyList<IRuntimeFallbackDefinition> fallbacks)
    {
        var lookup = new Dictionary<string, Dictionary<string, Dictionary<string, IRuntimeFallbackDefinition>>>(StringComparer.Ordinal);
        ...
        foreach (var fallback in fallbacks)
        {
            ...
            {
                alpnLookup = new Dictionary<string, Dictionary<string, IRuntimeFallbackDefinition>>(StringComparer.Ordinal);
                lookup[normalizedName] = alpnLookup;
            }

            if (!alpnLookup.TryGetValue(normalizedAlpn, out var pathLookup))
            {
                pathLookup = new Dictionary<string, IRuntimeFallbackDefinition>(StringComparer.Ordinal);
        ...
        if (lookup.TryGetValue(string.Empty, out var defaultNameLookup))
        {
            foreach (var (name, alpnLookup) in lookup)
            {
                ...
                foreach (var defaultAlpn in defaultNameLookup.Keys)
                {
                    ...
                    {
                        alpnLookup[defaultAlpn] = new Dictionary<string, IRuntimeFallbackDefinition>(StringComparer.Ordinal);
                    }
                }
            }
        }
    ...
    private static string SelectName(
        IReadOnlyDictionary<string, Dictionary<string, Dictionary<string, IRuntimeFallbackDefinition>>> lookup,
        string serverName)
    {
    ...
    private static string SelectAlpn(
        IReadOnlyDictionary<string, Dictionary<string, IRuntimeFallbackDefinition>> alpnLookup,
        string alpn)
        => alpnLookup.ContainsKey(alpn) ? alpn : string.Empty;

    private static string SelectPath(
        IReadOnlyDictionary<string, IRuntimeFallbackDefinition> pathLookup,
        ReadOnlySpan<byte> initialPayload)
    {
