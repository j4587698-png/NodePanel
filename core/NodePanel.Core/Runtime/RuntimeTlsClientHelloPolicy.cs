...
public interface IRuntimeTlsClientHelloPolicyDefinition
{
    bool Enabled { get; }
...
public sealed record RuntimeTlsClientHelloPolicyOptions : IRuntimeTlsClientHelloPolicyDefinition
{
    public static RuntimeTlsClientHelloPolicyOptions Disabled { get; } = new();
...
public sealed record RuntimeTlsClientHelloPolicyDecision
{
...
public static class RuntimeTlsClientHelloPolicyEvaluator
{
    public static bool ShouldReject(
        IRuntimeTlsClientHelloPolicyDefinition policy,
        RuntimeTlsClientHelloMetadata? metadata,
        out RuntimeTlsClientHelloPolicyDecision decision)
    {
        ArgumentNullException.ThrowIfNull(policy);

        if (!policy.Enabled)
        {
            decision = new RuntimeTlsClientHelloPolicyDecision();
            return false;
        }
    ...
    private static bool ContainsJa3(IReadOnlyList<string> configuredValues, RuntimeTlsClientHelloMetadata? metadata)
    {
