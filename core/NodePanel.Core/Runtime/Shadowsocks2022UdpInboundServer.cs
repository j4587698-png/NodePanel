...
public sealed class Shadowsocks2022UdpInboundServer
{
    ...
    public Shadowsocks2022UdpInboundServer(
    ...
    internal Shadowsocks2022UdpInboundServer(
        IDispatcher dispatcher,
        IRuntimeSessionRegistry sessionRegistry,
        IRuntimeRateLimiterRegistry rateLimiterRegistry,
        IRuntimeTrafficRegistry trafficRegistry,
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            throw new InvalidOperationException("Shadowsocks 2022 UDP inbound receive loop ended unexpectedly.");
        }
        finally
        {
            foreach (var listener in activeListeners)
        ...
        try
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    var transport = await EnsureTransportAsync(listener, session, packet, options).ConfigureAwait(false);
                    var destination = session.User.HasRelayDestination
                        ? new DispatchDestination
                        {
                            Host = session.User.Address,
        ...
        bool useCone)
    {
        ...
        lock (listener.Sync)
        {
            var key = CreateSessionKey(
                remoteEndPoint,
                packet,
                useCone || user.HasRelayDestination);
            if (listener.SessionsByKey.TryGetValue(key, out var existing))
            {
                if (!existing.LifetimeCts.IsCancellationRequested &&
                    ...
                    string.Equals(existing.Account.Method, account.Method, StringComparison.Ordinal))
                {
        ...
        ShadowsocksInboundServerOptions options)
    {
        ...
        try
        {
            ...
            var dispatchDestination = session.User.HasRelayDestination
                ? new DispatchDestination
                {
                    Host = session.User.Address,
                    Port = session.User.Port,
            ...
            var dispatchResult = await RuntimeUdpDispatchPipeline.DispatchAsync(
                ...
                allowDestinationOverride: !session.User.HasRelayDestination).ConfigureAwait(false);
            var transport = RuntimeSniffingUdpTransport.WrapIfNeeded(
                dispatchResult.Transport,
                _runtimeSniffer,
                sessionOptions.Sniffing,
                allowDestinationOverride: !session.User.HasRelayDestination);
    ...
    private static Socket CreateSocket(ListenerBinding binding)
    {
        if (binding.IsUnix)
        {
            throw new NotSupportedException("Shadowsocks 2022 UDP inbound does not support UNIX listeners.");
        }
    ...
    private static IPEndPoint GetRequiredLocalEndPoint(Socket socket)
        => socket.LocalEndPoint as IPEndPoint
           ?? throw new InvalidOperationException("Shadowsocks 2022 UDP socket is not using an IP endpoint.");
    ...
    private sealed class ListenerState
    {
        ...
        public Shadowsocks2022InboundRuntime Inbound { get; }
    ...
    private sealed class SessionState
    {
        ...
        public Shadowsocks2022User User { get; }

        public Shadowsocks2022Account Account { get; }
