...
public sealed class RuntimeFallbackRelayService
{
    ...
    public RuntimeFallbackRelayService(
    ...
    public async Task<bool> TryHandleAsync(
        Stream clientStream,
        byte[] initialPayload,
        IRuntimeFallbackConnectionOptions options,
        CancellationToken cancellationToken)
    {
        ...
        var fallback = RuntimeFallbackSelector.Select(
            options.Fallbacks,
            options.ServerName,
            options.Alpn,
            initialPayload);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var networkType = NormalizeNetworkType(target.Type);
        await using var remoteStream = await OpenFallbackStreamAsync(networkType, target.Destination, cancellationToken).ConfigureAwait(false);

        if (target.ProxyProtocolVersion is 1 or 2)
        {
            var proxyHeader = BuildProxyProtocolHeader(
                target.ProxyProtocolVersion,
        ...
        Stream relayClientStream = clientStream;
        Stream relayRemoteStream = remoteStream;
        if (target.LimitUpload is { IsEnabled: true } limitUpload && !limitUpload.IsEmpty)
        {
            relayClientStream = new RuntimeFallbackLimitedStream(clientStream, limitUpload);
        }

        if (target.LimitDownload is { IsEnabled: true } limitDownload && !limitDownload.IsEmpty)
        {
            relayRemoteStream = new RuntimeFallbackLimitedStream(remoteStream, limitDownload);
        }

        await _relayService.RelayAsync(relayClientStream, relayRemoteStream, cancellationToken).ConfigureAwait(false);
    }
    ...
    private async Task<Stream> OpenFallbackStreamAsync(string networkType, string destination, CancellationToken cancellationToken)
    {
        return networkType switch
        {
            ...
            TrojanFallbackCompatibility.ServeNetworkType => throw new NotSupportedException("Runtime fallback transport type 'serve' is not supported by the current .NET runtime."),
            _ => throw new NotSupportedException($"Unsupported runtime fallback transport type: {networkType}.")
        };
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        throw lastError ?? new IOException("Runtime fallback connection failed.");
    }
    ...
    private async Task<Stream> OpenTcpFallbackStreamAsync(
        ...
        CancellationToken cancellationToken)
    {
        var dialContext = new DispatchContext
        {
    ...
    private static async Task<Stream> OpenUnixFallbackStreamAsync(string destination, CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidDataException("Runtime fallback destination is empty.");
        }
    ...
    private static FallbackTcpDestination ParseTcpDestination(string networkType, string value)
    {
        ...
        {
            throw new InvalidDataException($"Runtime fallback destination is invalid: {value}.");
        }
