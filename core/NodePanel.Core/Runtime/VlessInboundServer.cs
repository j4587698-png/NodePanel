...
public sealed class VlessInboundServer
{
    ...
    public async Task RunAsync(VlessInboundServerOptions options, CancellationToken cancellationToken)
    {
        ...
        var tlsOptions = options.Tls ?? throw new InvalidOperationException("VLESS inbound server requires TLS options when listeners are configured.");
        ...
        try
        {
            ...
            throw new InvalidOperationException("VLESS inbound accept loop ended unexpectedly.");
        }
        finally
        {
            foreach (var listener in activeListeners)
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var inbound = listener.RawTlsInbound ?? listener.WebSocketInbound ?? throw new InvalidOperationException("VLESS listener has no inbound definition.");
        ...
        var inboundSelection = VlessInboundRuntimePlanner.SelectInbound(listener, initialPayload)
            ?? throw new InvalidOperationException("VLESS shared listener could not resolve an inbound transport.");
        ...
        CancellationToken cancellationToken)
    {
        if (string.Equals(inbound.Transport, InboundTransports.Wss, StringComparison.Ordinal))
        {
            ...
            await _vlessInboundConnectionHandler.HandleAsync(
                webSocketStream,
    ...
    private static VlessInboundSessionOptions CreateSessionOptions(
        VlessInboundServerOptions options,
        VlessTlsInboundRuntime inbound,
        string serverName,
        string alpn,
    ...
    private sealed record ListenerRuntime(VlessTlsListenerRuntime Definition, ListenerHandle Handle);
}
