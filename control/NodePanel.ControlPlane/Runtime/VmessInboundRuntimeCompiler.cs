...
public sealed class VmessInboundRuntimeCompiler : IInboundProtocolRuntimeCompiler
{
        ...
        out string? error)
    {
        ...
        if (!VmessInboundRuntimePlanner.TryBuild(
                config.Inbounds.Cast<IVmessInboundDefinition>().ToArray(),
                out var plan,
                out error))
        {
    ...
    private static InboundConfig NormalizeInbound(InboundConfig inbound)
    {
        ...
        if (!string.Equals(protocol, InboundProtocols.Vmess, StringComparison.Ordinal))
        {
            return inbound;
        }
        ...
        => inbounds
            .Where(static inbound => inbound.Enabled &&
                                     (NodeServiceConfigInbounds.IsProtocolTransport(inbound, InboundProtocols.Vmess, InboundTransports.Tls) ||
                                      ...
                                      NodeServiceConfigInbounds.IsProtocolTransport(inbound, InboundProtocols.Vmess, InboundTransports.SplitHttp)))
            .SelectMany(static inbound => inbound.Users
                .Where(static user => !string.IsNullOrWhiteSpace(user.UserId) && TryNormalizeUuid(user.Uuid, out _))
                .Select(user => new VmessUser
                {
                    UserId = user.UserId.Trim(),
                    Uuid = NormalizeUuid(user.Uuid),
                    CmdKey = VmessAccountCodec.CreateCommandKey(NormalizeUuid(user.Uuid)),
                    RuntimeKey = RuntimeUserKeys.Create(InboundProtocols.Vmess, inbound.Tag, user.UserId),
                    Level = Math.Max(0, user.Level),
                    BytesPerSecond = Math.Max(0, user.BytesPerSecond),
                    DeviceLimit = Math.Max(0, user.DeviceLimit)
                }))
