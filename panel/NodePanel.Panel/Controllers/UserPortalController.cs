...
public sealed class UserPortalController : Controller
{
    ...
    {
        var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
        if (string.IsNullOrWhiteSpace(userId)) return Redirect("/auth/login");
    ...
    public async Task<IActionResult> Checkout(string planId, string cycle, CancellationToken cancellationToken)
    {
        ...
        decimal amount = cycle switch
        {
            ...
            "reset_price" => plan.ResetPrice ?? 0,
            _ => 0
        };

        if (amount <= 0 && cycle != "free")
        {
        ...
        TempData["StatusMessage"] = $"订单已创建，状态为待支付。请点击模拟支付完成结账。";
