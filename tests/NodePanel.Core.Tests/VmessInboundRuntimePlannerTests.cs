...
public sealed class VmessInboundRuntimePlannerTests
{
    [Fact]
    public void TryBuild_supports_grpc_tls_binding_and_selects_grpc_by_http2_preface()
    {
        var result = VmessInboundRuntimePlanner.TryBuild(
        ...
        Assert.Equal("/vmess/service/Tun|TunMulti", listener.GrpcInbound!.Grpc.ServiceName);
        Assert.Equal(["h2"], listener.GrpcInbound.ApplicationProtocols);

        var selected = VmessInboundRuntimePlanner.SelectInbound(
            listener,
    ...
    public void TryBuild_supports_splithttp_tls_binding_and_selects_by_http_path_prefix()
    {
        ...
        var splitHttpInbound = VmessInboundRuntimePlanner.SelectInbound(
            listener,
            Encoding.ASCII.GetBytes("GET /xhttp/session-1 HTTP/1.1\r\nHost: edge.example.com\r\n\r\n"));
        var rawTlsInbound = VmessInboundRuntimePlanner.SelectInbound(
            listener,
