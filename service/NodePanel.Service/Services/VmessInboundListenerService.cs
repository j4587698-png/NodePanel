...
public sealed class VmessInboundListenerService : ReloadingInboundHostedServiceBase
{
    private readonly ILogger<VmessInboundListenerService> _logger;
    private readonly VmessInboundServer _vmessInboundServer;
    private readonly XrayRuntimeOptions _xrayRuntimeOptions;

    public VmessInboundListenerService(
        RuntimeConfigStore runtimeConfigStore,
        CertificateStateStore certificateStateStore,
        VmessInboundServer vmessInboundServer,
        XrayRuntimeOptions xrayRuntimeOptions,
        ILogger<VmessInboundListenerService> logger)
        : base(runtimeConfigStore, certificateStateStore, logger)
    {
        _vmessInboundServer = vmessInboundServer;
        _xrayRuntimeOptions = xrayRuntimeOptions;
        _logger = logger;
    }

    protected override string HostDisplayName => "VMess inbound server";
    ...
    protected override bool RequiresCertificate(NodeRuntimeSnapshot snapshot)
        => snapshot.GetInboundPlanOrDefault(InboundProtocols.Vmess, VmessInboundRuntimePlan.Empty).RequiresCertificate;
        ...
        CancellationToken cancellationToken)
    {
        ...
        return _vmessInboundServer.RunAsync(CreateServerOptions(snapshot, certificate), cancellationToken);
    }

    private VmessInboundServerOptions CreateServerOptions(
        NodeRuntimeSnapshot snapshot,
        System.Security.Cryptography.X509Certificates.X509Certificate2 certificate)
        => new()
        {
            Plan = snapshot.GetInboundPlanOrDefault(InboundProtocols.Vmess, VmessInboundRuntimePlan.Empty),
            ...
            Callbacks = new VmessInboundServerCallbacks
            {
                ...
                ClientHelloRejected = context => _logger.LogWarning(
                    "Rejected VMess inbound connection from {RemoteEndPoint} due to client hello policy '{Reason}' (SNI: {ServerName}, JA3: {Ja3Hash}).",
                ...
                UnknownServerNameRejected = context => _logger.LogWarning(
                    "Rejected VMess inbound connection from {RemoteEndPoint} due to unknown SNI '{ServerName}'.",
                    context.RemoteEndPoint,
                ...
                ConnectionError = context => _logger.LogDebug(
                    context.Exception,
                    "VMess inbound connection failed from {RemoteEndPoint}.",
                    context.RemoteEndPoint)
            }
        };
    ...
    private void LogListenerStart(VmessTlsListenerRuntime listener, int revision)
    {
